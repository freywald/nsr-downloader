...
#### 2. Start download.
When done with collecting links, run ``nsr-downloader`` to start processing of the default queues [audio, video, audio+video, audio_playlist]
...
You need a wrapper script that calls nsr-downloader and a link list.
...
nsr-downloader --from-file="$link_list_locator" --target-directory="$target_directory" "$@"
...
1. ```$ cd $HOME/bin && git clone git@github.com:freywald/nsr-downloader.git nsr-downloader && cd nsr-downloader && ./install && echo "Installation successful."```
